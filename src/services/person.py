...
class PersonService:
    ...
    async def get_person_by_id(self, person_id: uuid.UUID) -> Optional[Person]:
        ...
        if not person:
            ...
            if not person:
                return None
            await self._put_person_to_cache(person.model_dump_json(), str(person_id))
    ...
    async def get_persons_by_query(self,
        ...
        if not persons:
            ...
            if not persons:
                return []
            value = json.dumps([person.json() for person in persons])
    ...
    async def _person_from_cache(self, key: str) -> None | Person | list[Person] | Any:
