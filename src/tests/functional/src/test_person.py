...
from ..testdata.es_data import es_data, es_persons_data, es_person_films_data
...
# #             {'status': HTTP_200,
# #              'body': [{
# #                  'uuid':  es_person_films_data[0].get('id'),
# #                  'title': 'War',
# #                  'imdb_rating': 10
# #              },]
# #              }
...
#     # await es_write_data(es_person_films_data, test_settings.es_movies_index)
#     # await es_write_data(es_persons_data, test_settings.es_persons_index)
...
@pytest.mark.asyncio
async def test_person_cach(es_write_data, make_get_request, query_data, expected_answer):
    await es_write_data(es_persons_data)
    await make_get_request(f"persons/{query_data.get('id')}")
    ElasticPersonHandler.get_person_by_id = False
    response = await make_get_request(f"persons/{query_data.get('id')}")
